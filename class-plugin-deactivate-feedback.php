...
if( ! class_exists( 'SWM_Usage_Feedback') ) {
	...
	class SWM_Usage_Feedback {
		...
		public function deactivate_this_plugin() {
			...
			if( false !== get_option( 'swm_deactivation_reason_' . $this->plugin_name ) ) {
				$body['deactivation_reason'] = get_option( 'swm_deactivation_reason_' . $this->plugin_name );
				delete_option('swm_deactivation_reason_' . $this->plugin_name);
			}
			if( false !== get_option( 'swm_deactivation_details_' . $this->plugin_name ) ) {
				$body['deactivation_details'] = get_option( 'swm_deactivation_details_' . $this->plugin_name );
				delete_option('swm_deactivation_details_' . $this->plugin_name);
			}

			if( false !== get_option( 'swm_deactivation_email_' . $this->plugin_name ) ) {
				$body['deactivation_email'] = get_option( 'swm_deactivation_email_' . $this->plugin_name );
				delete_option('swm_deactivation_email_' . $this->plugin_name);
			}

			if( false !== get_option( 'swm_deactivation_main_reason_' . $this->plugin_name ) ) {
				$body['deactivation_main_reason'] = get_option( 'swm_deactivation_main_reason_' . $this->plugin_name );
				delete_option('swm_deactivation_main_reason_' . $this->plugin_name);
			}
		...
		public function goodbye_ajax() {
			...
			if( is_array( $form['options'] ) ) {
				...
				$html .= '</p><div id="swm_additional_content" style="display:none;"><label for="swm-goodbye-reasons">' . esc_html( $form['email'] ) .'</label><br><input type="email" name="swm-goodbye-email" id="swm-goodbye-email" value="'.get_option('admin_email').'" /> (Optional)';
		...
		public function goodbye_form_callback() {
			...
			if( isset( $_POST['values'] ) ) {
				...
				update_option( 'swm_deactivation_reason_' . $this->plugin_name, $values );
			}
			if( isset( $_POST['details'] ) ) {
				$details = sanitize_text_field( $_POST['details'] );
				update_option( 'swm_deactivation_details_' . $this->plugin_name, $details );
			}

			if( isset( $_POST['email'] ) ) {
				$email = sanitize_text_field( $_POST['email'] );
				update_option( 'swm_deactivation_email_' . $this->plugin_name, $email );
			}

			if( isset( $_POST['main_reason'] ) ) {
				$main_reason = sanitize_text_field( $_POST['main_reason'] );
				update_option( 'swm_deactivation_main_reason_' . $this->plugin_name, $main_reason );
			}
