...
class SWM_Shopify_Product
{
	...
	public function get_all_shopify_products(){
		...
		$result_per_request = get_option('wb_swm_result_per_request') ? get_option('wb_swm_result_per_request') : SWM_DEFAULT_RESULT_PER_REQUEST;
		...
		$import_records = get_option('swm_import_records', $import_records_options);
		...
		update_option('swm_import_records', $swm_import_records);

		$response['swm_import_records'] = get_option('swm_import_records', $import_records_options);
			
		return $response;
		
	}

	public function get_product_by_collection_id($collection_id){
		$total_products = $this->count_total_product_by_collection_id($collection_id);
		$product_ids = array();
		$pages = ceil( $total_products['count'] / 250 );
		for ($i=1; $i<=$pages ; $i++) { 
			$url = swm_set_store_url_query() . 'products.json';
			$url = add_query_arg(array(
			...
			$products = isset($products_by_collection_id['products']) ? $products_by_collection_id['products'] : array();
