...
class SWM_WC_Product
{
	...
	public function import_wc_product($shopify_product_array){
		...
		for( $i = 0; $i < count($shopify_product_array); $i++ ){
			...
			$post_default_status = swm_get_option( 'wb_swm_product_status' ) ? swm_get_option( 'wb_swm_product_status' ) : 'publish';
			$product_categories = swm_get_option( 'wb_swm_product_categories' ) ? swm_get_option( 'wb_swm_product_categories' ) : '';
			$default_product_tags = swm_get_option( 'wb_swm_product_tags' ) ? swm_get_option( 'wb_swm_product_tags' ) : array();
			$post_params = array(
			    'post_type' => "product",
			...
			if( $existing_product_id ){
				//Update Product
				$post_params['ID'] = $existing_product_id;
			...
			if( !is_wp_error($post_id) && $post_id ){
				...
				if( count($shopify_product['variants']) == 1 ){
					...
					$this->create_simple_product($post_id, $shopify_product);
				}elseif ( count($shopify_product['variants']) >1 ) {
					//$import_result .= '<strong>variable </strong> ';
					$this->create_variable_product($post_id, $shopify_product);
				}
	...
	public function create_simple_product($post_id, $shopify_product){
		$wb_swm_download_images = get_option('wb_swm_download_images') ? get_option('wb_swm_download_images') : SWM_DEFAULT_IMAGE_DOWNLOAD_STATUS;
	...
	public function create_variable_product($post_id, $shopify_product){
		$wb_swm_download_images = get_option('wb_swm_download_images') ? get_option('wb_swm_download_images') : SWM_DEFAULT_IMAGE_DOWNLOAD_STATUS;
