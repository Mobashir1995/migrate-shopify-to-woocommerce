 ...
 *******************************/
add_action('wp_head', 'swm_support_form_ajax_header');

if( !function_exists('swm_support_form_ajax_header') )
{
	function swm_support_form_ajax_header() 
	{
	...
	} //End of swm_support_form_ajax_header
 ...
 *******************************/
add_action( 'wp_ajax_process_swm_promo_form', 'process_swm_promo_form' );

if( !function_exists('process_swm_promo_form') )
{
	function process_swm_promo_form()
	{
...
Please email to <span style="color:#22A0C9;font-weight:bold !important;font-size:14px "> webbuilders03@gmail.com </span> with any feedback. We will get back to you right away!', '');
		...
		else
		{
			...
			//$toEmail = "swm.kadir@gmail.com"; //Receivers email address
 ...
 *******************************/
if( !class_exists('SWMSupportPage') ){


	class SWMSupportPage{
	
		public $plugin_menu_slug = "";
		public $plugin_slug = ""; //Should be unique, like: swmsld_p123
		...
		function __construct( $plugin_slug = null )
		{
			...
			//add_action( 'wp_ajax_process_swm_promo_form', array(&$this,'process_swm_promo_form') );
			
		} //End of Constructor
		
		function include_promo_page_scripts( $hook )
		{                                 
		   if( $hook == 'shopify-to-wc_page_wb-page-swm-support' ){	   	
		...
		function show_promo_page_callback_func()
		{
			add_submenu_page(
				...
				'manage_options',
				$this->page_slug,
				array(&$this, 'swmpromo_support_page_callback_func' )
			);
		 ...
		 *******************************/
		function swmpromo_support_page_callback_func()
		{
			
			?>
				
				<div class="swm-support-page-wrapper">
								...
								<div class="container-wrapper">
									...
									<div class="contact-support-left">
										...
										<div class="support-form">
											<form class="form" id="swm-support-form" method="POST">
											  ...
											  <div class="submit">
												<input type="submit" value="Send Message" class="button-blue" id="swm-query-submit-btn"/>
												<div class="ease"></div>
											  </div>
											</form>
										</div>
			...
			<?php
		} //End of swmpromo_support_page_callback_function
		
		
	
	} //End of the class SWMSupportPage
...
//Supply Unique Promo Page Slug as the constructor parameter of the class SWMSupportPage. ex: sld-page-2124a to the constructor
